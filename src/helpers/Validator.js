...
module.exports = class Validator {
  static validateConfiguration() {
    log("Validation du fichier de configuration et des variables d'environnement");
    ...
    if (!process.env.MONGO_CONNECTION) {
      error("env: MONGO CONNECTION ne peut pas être vide");
      process.exit(1);
    }
    ...
    if (config.DASHBOARD.enabled) {
      ...
      if (!config.DASHBOARD.baseURL || !config.DASHBOARD.failureURL || !config.DASHBOARD.port) {
        error("config.js:  Les détails du DASHBOARD ne peuvent pas être vides");
        process.exit(1);
      }
    }
    ...
    if (isNaN(config.CACHE_SIZE.GUILDS) || isNaN(config.CACHE_SIZE.USERS) || isNaN(config.CACHE_SIZE.MEMBERS)) {
      error("config.js: CACHE_SIZE doit être un entier positif");
      process.exit(1);
    }
    ...
    if (config.MUSIC.ENABLED) {
      if (!process.env.SPOTIFY_CLIENT_ID || !process.env.SPOTIFY_CLIENT_SECRET) {
        warn("env: SPOTIFY_CLIENT_ID or SPOTIFY_CLIENT_SECRETsont manquants. Les liens musicaux Spotify ne fonctionnent pas");
      }
      if (config.MUSIC.LAVALINK_NODES.length == 0) {
        warn("config.js: Il doit y avoir au moins un nœud pour Lavalink");
      }
    ...
    // Warnings
    if (config.OWNER_IDS.length === 0) warn("config.js: OWNER_IDS sont vides");
    if (!config.SUPPORT_SERVER) warn("config.js: SUPPORT_SERVER n'est pas fourni");
    if (!process.env.WEATHERSTACK_KEY) warn("env: WEATHERSTACK_KEY est manquant. La commande météo ne fonctionnera pas");
    if (!process.env.STRANGE_API_KEY) warn("env: STRANGE_API_KEY est manquant. Les commandes d'image ne fonctionneront pas");
  }
  ...
  static validateCommand(cmd) {
    if (typeof cmd !== "object") {
      throw new TypeError("Les données de commande doivent être un objet.");
    }
    if (typeof cmd.name !== "string" || cmd.name !== cmd.name.toLowerCase()) {
      throw new Error("Le nom de la commande doit être une chaîne minuscule.");
    }
    if (typeof cmd.description !== "string") {
      throw new TypeError("La description de la commande doit être une chaîne.");
    }
    if (cmd.cooldown && typeof cmd.cooldown !== "number") {
      throw new TypeError("Le temps de recharge des commandes doit être un nombre");
    }
    if (cmd.category) {
      if (!Object.prototype.hasOwnProperty.call(CommandCategory, cmd.category)) {
        throw new Error(`Pas de catégorie valide ${cmd.category}`);
      }
    }
    if (cmd.userPermissions) {
      if (!Array.isArray(cmd.userPermissions)) {
        throw new TypeError("Les autorisations de l’utilisateur de commande doivent être un tableau de chaînes de clés d’autorisation.");
      }
    ...
    if (cmd.botPermissions) {
      if (!Array.isArray(cmd.botPermissions)) {
        throw new TypeError("Les autorisations du robot de commande doivent être un tableau de chaînes de clés d’autorisation.");
      }
